...
JS语言的一大特点就是单线程，也就是说同一时间只能做一件事。单线程就意味着，所有任务需要排队，前一个任务结束，才会执行后一个任务。这样所导致的问题是: 如果JS执行的事件过长，这样就会造成页面的渲染不连贯，导致页面渲染阻塞的感觉。

JS中出现同步和异步: 

- 同步: 前一个任务结束后再执行后一个人俗，程序的执行顺序与任务的排序顺序是一致的、同步的。
- 异步: 做一件事情耗时很长，在做这件事的同时，还可以去处理其他事。JS的异步是通过回调函数实现的。

#### 事件循环

- 定义: 由于主线程不断地重复获得任务、执行任务、再获取任务、再执行，所以这种机制被称为事件循环(Event loop)
...
- `map`可以遍历数组处理数据，并且返回新的数组。`map`也被称为映射，重点在于有返回值，`forEach`没有返回值。

- `join`方法用于吧数组中的所有元素转换成一个字符串
...
- 引用计数法
  - 缺点: 嵌套引用(循环引用) 两个对象相互引用，尽管他们已不再适用，垃圾回收器不会进行回收，导致内存泄漏。计数就永远不是为0，所以对象无法被清空(无法被回收)

- 标记清除: 给当前不使用的值加上标记，然后再回收他的内存
  - 现在浏览器已经不再使用引用计数算法了
...
- 简单理解: 闭包 = 内层函数 + 外层函数的变量。只是两个函数的嵌套不会产生闭包，而是形成嵌套情况，然后内层函数使用到了外层函数的变量(捆绑到一起)，这个时候才会产生闭包。
