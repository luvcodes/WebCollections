...
### 事件流 - 代码可见`JavaScript.2_PhaseTwo.DOM.4_EventsStream`

#### 事件流: 指的是事件完整执行过程中的流动路径

#### 事件捕获

- 概念: **事件捕获刚好就和事件冒泡反过来。**事件捕获就是：**当元素被触发事件时候，从该元素的根节点开始逐级向里寻找同类型事件。这个过程，就被称为事件捕获**
- 在事件捕获的状态下，三个div的输出顺序是：div1 > div2 > div3

#### 事件冒泡

- 概念: 当某元素执行某一种类型事件，那么从该元素起逐级向外层的元素检测是否存在与本身同样的事件。这一个过程，就叫做事件冒泡。
- 在事件冒泡的前提下，三个嵌套div的输出顺序是：div1 > div2 > div3
...
### `setTimeout`和`setInterval`函数的区别

- setTimeout() 在于载入后延迟指定的时间后才会**执行一次**表达式，执行过后便会停止；`setTimeout`只会执行一次，所以可以理解为就是把一段代码延迟执行，平时省略window (也就是`window.setTimeout`)
...
- setInterval() 在载入后每隔指定时间就会执行一次表达式，直到定时器或者窗口关闭。setInterval() 会**重复执行多次**。如果想**只执行一次**，请**使用 setTimeout()**
